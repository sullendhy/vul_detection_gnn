...
class GCN_GGNN(nn.Module):
    def __init__(self, feature_dim_size, hidden_size, num_GNN_layers, dropout, act=nn.functional.relu,
                 residual=True, att_op='sum', alpha_weight=1.0):
        super(GCN_GGNN, self).__init__()
        self.num_GNN_layers = num_GNN_layers
        self.residual = residual
        self.att_op = att_op
        self.alpha_weight = alpha_weight
        ...
        for layer in range(self.num_GNN_layers):
            if layer == 0:
                self.gcn_layers.append(GraphConvolution(feature_dim_size, hidden_size, dropout, act=act))
            else:
                self.gcn_layers.append(GraphConvolution(hidden_size, hidden_size, dropout, act=act))

        # ReGGNN
        self.emb_encode = nn.Linear(hidden_size, hidden_size).double()
        self.dropout_encode = nn.Dropout(dropout)
        self.z0 = nn.Linear(hidden_size, hidden_size).double()
        self.z1 = nn.Linear(hidden_size, hidden_size).double()
        self.r0 = nn.Linear(hidden_size, hidden_size).double()
    ...
    def forward(self, inputs, adj, mask):
        ...
        for idx_layer in range(self.num_GNN_layers):
            if idx_layer == 0:
                ...
                if self.residual:
                    x = x + self.gcn_layers[idx_layer](x, adj) * mask  # Residual Connection, can use a weighted sum
                else:
                    x = self.gcn_layers[idx_layer](x, adj) * mask
        ...
        soft_att = torch.sigmoid(self.soft_att(x))
        x = self.act(self.ln(x))
        x = soft_att * x * mask       # x [16, 变化, 256]

        # weighted pooling (sum and max)
        graph_embeddings = 0.9 * torch.sum(x, 1) + 0.1 * torch.amax(x, 1)  # 
        return graph_embeddings    # graph_embeddings [16, 256]
...
def build_graph_text(shuffle_doc_words_list, word_embeddings, position_encodings, window_size=3):
    # print('using window size = ', window_size)
    x_adj = []
    x_feature = []
